...
def _score_order(
    ...
    """Return the three-layer Score plus total lateness as guidance diagnostics."""
    ...
    return Score(late_count, total_lateness, distance), total_lateness
...
def _build_pair_interaction(
    left: Task,
    right: Task,
    score_order: Callable[[tuple[int, ...]], tuple[Score, float]],
) -> PairInteraction:
    ...
    (joint_score, joint_lateness), best_order = min(scored_orders)
    (separate_left, separate_left_lateness) = score_order((left.id, -left.id))
    (separate_right, separate_right_lateness) = score_order((right.id, -right.id))
    separate_score = separate_left + separate_right
    separate_lateness = separate_left_lateness + separate_right_lateness
    ...
    extra_lateness = max(0.0, joint_lateness - separate_lateness)
...
def _evaluate_problem_pair_cached(
    ...
    def score_order(order: tuple[int, ...]) -> tuple[Score, float]:
