...
class ALNSConfig:
    ...
    relay_leg_beam: int = 6
    relay_event_cap: int | None = 60
    relay_sample_every: int = 4
    relay_global_limit: int = 3
    relay_debug: bool = False
    enable_relay_refine: bool = False
    relay_refine_interval: int = 5
    relay_refine_task_limit: int | None = 16
...
def _relay_refine_pass(
    ...
    *,
    ...
    if not relay_tasks:
        return routes
    current = routes
    current_score = global_evaluator.evaluate(current).score
    considered = 0
    for task_id in sorted(relay_tasks):
        ...
        for leg_id, source_route, other_route, is_out in (
            ...
            for target_index, target in enumerate(current):
                ...
                for option in options:
                    ...
                    schedule = global_evaluator.evaluate(candidate_routes)
...
def destroy_solution(
    ...
    *,
    ...
    def removal_gain(task_id: int) -> Score:
        visits = frozenset(plan_index.task_visits[task_id])
        gain = Score(0, 0.0, 0.0)
        for route_index in plan_index.task_routes[task_id]:
            reduced = tuple(
                visit for visit in routes[route_index] if visit not in visits
            )
            gain += (
                route_metrics[route_index].score
...
class _RepairCache:
    ...
    __slots__ = ("versions", "surfaces", "direct", "leg", "combos")

    def __init__(self, route_count: int) -> None:
        self.versions = [0] * route_count
        self.surfaces: dict[
            tuple[int, int], RouteInsertionSurface
        ] = {}
        self.direct: dict[
    ...
    def surface(
        self, problem: Problem, routes: Routes, route_index: int
    ) -> RouteInsertionSurface:
        key = (route_index, self.versions[route_index])
        surface = self.surfaces.get(key)
        if surface is None:
            surface = route_insertion_surface(problem, routes[route_index])
    ...
    def direct_options(
        ...
        if deadline is not None and perf_counter() >= deadline:
            raise _SearchDeadlineReached
        surface = self.surface(problem, routes, route_index)
...
def _build_relay_beam(
    ...
    for in_opt in inbound:
        for out_opt in outbound:
            if (
                ...
                continue
            # Handoff-aware cheap surrogate: estimate the waiting at the
            # relay and its impact on the outbound delivery before any
            # global evaluation.  Demotes combos that force a large wait.
    ...
    return tuple(
        (score, in_opt, out_opt)
        for score, _, in_opt, out_opt in combos[: relay_cfg.plan_beam]
    )
...
def _apply_task_exact(
    ...
    *,
    ...
    with the best relay option's top ``global_limit`` beam combinations.
    ...
    for option in options:
        ...
        local_delta = (
            evaluator.evaluate(option.direct_route).score
            - evaluator.evaluate(
                routes[option.direct_route_index]
            ).score
        )
        if best_score is None:
            current_score = global_evaluator.evaluate(routes).score
        else:
            # The local delta is a safe lower bound of the true global
            # delta (waiting only ever delays deliveries), so a candidate
            # whose local estimate cannot beat the incumbent is skipped.
            if current_score is not None and (
                current_score + local_delta
            ) >= best_score:
                continue
        if not _route_has_relay_event_at_or_after(
        ...
        if best_score is None or candidate_score < best_score:
            ...
            best_mode = "DIRECT"
    relay_option = next(
        (option for option in options if option.mode == "RELAY"), None
    )
    if relay_option is not None:
        legs = tuple(
            leg
        ...
        for in_opt, out_opt in relay_option.combos[: relay_cfg.global_limit]:
            if deadline is not None and perf_counter() >= deadline:
                raise _SearchDeadlineReached
            if best_score is not None and current_score is not None:
                estimate = current_score + _handoff_surrogate(
                    problem, in_opt, out_opt, task_deadline
                )[0]
                if estimate >= best_score:
                    # The handoff surrogate is a lower bound of the true
                    # global score: the combo cannot beat the incumbent.
                    continue
            ...
            if candidate is None:
                continue
            schedule = global_evaluator.evaluate(candidate)
            relay_cfg.global_evaluation_count += 1
