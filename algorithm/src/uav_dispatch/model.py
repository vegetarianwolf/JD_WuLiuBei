...
class Problem:
    ...
    tasks: tuple[Task, ...]
    drone_count: int = 1
    drone_homes: tuple[int | None | Point, ...] | None = None
    ...
    drone_home_nodes: tuple[int, ...] = field(
    ...
    def __post_init__(self) -> None:
        ...
        if self.drone_homes is None:
            home_nodes = (0,) * self.drone_count
        else:
            homes = tuple(self.drone_homes)
            if len(homes) != self.drone_count:
                raise ValueError(
                    f"无人机起点数量 {len(homes)} 必须等于无人机数 "
                    f"{self.drone_count}"
                )
            # Arbitrary-coordinate homes (e.g. k-means cluster centres) are
            # registered as extra distance-matrix nodes before the matrix is
            # built, so every drone home is reachable through home_node().
            point_home_nodes: dict[tuple[float, float], int] = {}
            for home in homes:
                if isinstance(home, Point):
                    key = (home.x, home.y)
                    if key not in point_home_nodes:
                        point_home_nodes[key] = len(points)
                        points.append(home)
            home_nodes_list: list[int] = []
            for index, home in enumerate(homes):
                if home is None:
                    home_nodes_list.append(0)
                elif isinstance(home, Point):
                    home_nodes_list.append(point_home_nodes[(home.x, home.y)])
                elif home in relay_to_node:
                    home_nodes_list.append(relay_to_node[home])
                else:
                    raise ValueError(
                        f"无人机 {index + 1} 的起点引用未知中继站 {home}"
                    )
            home_nodes = tuple(home_nodes_list)
        object.__setattr__(self, "drone_home_nodes", home_nodes)
    ...
    def home_node(self, route_index: int) -> int:
        """Distance-matrix node where drone ``route_index`` starts (t=0)."""

        return self.drone_home_nodes[route_index]

    def home_points(self) -> tuple[Point, ...]:
        """Euclidean coordinates of each drone's starting point (t=0)."""

        homes = (
            self.drone_homes
            if self.drone_homes is not None
            else (None,) * self.drone_count
        )
        station_by_id = {
            station.id: station for station in self.relay_stations
        }
        points: list[Point] = []
        for home in homes:
            if home is None:
                ...
                points.append(station_by_id[home].point)
        return tuple(points)
    ...
    def distance(
        ...
        *,
        ...
        if from_visit is None:
            from_index = 0 if from_node is None else from_node
        else:
            from_index = node_map[from_visit]
        return self._distances[from_index][node_map[to_visit]]
