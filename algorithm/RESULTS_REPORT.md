...
在 8 架无人机、每机最多 25 项任务的正式配置下，4 分钟限制内的最好方案在批量实验中用时 64.72 秒，取得词典序得分 **(76 项逾期，2860.03 分钟总逾期，694.69 km)**，按时率为 **62.0%**。相较 Regret-2 初始解，逾期任务减少 32 项，按时率提高 16.0 个百分点，总逾期下降 35.93%，总里程下降 10.45%。该方案的 200 项任务均且仅被服务一次，满足同机取送、先取后送、载荷不超过 2、每机不超过 25 项任务等约束，并通过了不复用搜索缓存的独立校验器。

另一次 1500 轮离线扩展搜索得到 **(67, 2272.19, 641.64)**，但耗时 313.31 秒，超过 4 分钟，因此只作为算法潜力参考，不计为时限内成绩。
...
正式求解读取 GB18030 编码的 CSV，原始 `.xls` 和用户转换的 `.xlsx` 同时保留以便审计。CSV 的 SHA-256 为 `cdc9f08afa39f53407b7e30157eeda2e89a10bbc72dd3b1d2ce19e832caa98e3`，XLSX 的 SHA-256 为 `2c39a150d483ed8b96b4ad550b31bf8344d63883ca25678943d847c19af10c37`。
...
| C2-Lex-ALNS，均值 ±95% CI 半宽 | 5 | 13.8 ±0.56 | 44.8% ±2.22 pp | 545.97 ±31.74 | 100.11 ±3.46 | 2.12 ±0.20 |
| C2-Lex-HALNS，均值 ±95% CI 半宽 | 5 | 13.8 ±0.56 | 44.8% ±2.22 pp | 545.97 ±31.74 | 100.11 ±3.46 | 2.12 ±0.17 |
...
| C2-Lex-ALNS，均值 ±95% CI 半宽 | 5 | 83.4 ±4.17 | 58.3% ±2.09 pp | 3056.27 ±354.74 | 716.34 ±41.32 | 54.41 ±2.98 |
| C2-Lex-HALNS，均值 ±95% CI 半宽 | 5 | 80.6 ±3.98 | 59.7% ±1.99 pp | 2970.60 ±226.47 | 714.34 ±21.52 | 63.76 ±3.45 |
| C2-Lex-HALNS，4 分钟内最好 | 1 | **76** | **62.0%** | **2860.03** | **694.69** | **64.72** |
| C2-Lex-HALNS，1500 轮离线扩展 | 1 | 67 | 66.5% | 2272.19 | 641.64 | 313.31（超时） |
...
上表采用正式 5 种子批量日志中该配置的 64.72 秒墙钟时间。为导出完整路线而执行的同种子独立 CLI 复跑得到相同词典序得分，记录为 58.44 秒；墙钟差异反映运行负载波动，不代表算法结果变化。两次运行均低于 240 秒。
