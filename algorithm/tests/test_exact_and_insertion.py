...
def test_exact_pareto_dp_matches_direct_enumeration_on_random_instances():
    ...
    for seed in range(5):
        ...
        # The exact DP optimises (late_count, distance_km) — total_lateness
        # is a derived diagnostic.  Assert on the two layers that the DP
        # actually optimises.
        assert actual.evaluation.score.late_count == expected_score.late_count
        assert actual.evaluation.score.distance_km <= expected_score.distance_km + 1e-9
        assert len(actual.routes[0]) == 2 * len(problem.tasks)
...
def test_exact_dominance_never_discards_a_slightly_earlier_future_clock():
    ...
    assert result.evaluation.total_lateness_min == pytest.approx(4.0)
