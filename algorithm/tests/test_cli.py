...
def test_cli_swap_method_emits_swap_model_block(tmp_path):
    source = tmp_path / "tasks.csv"
    source.write_text(
        "task_id,pickup_x,pickup_y,delivery_x,delivery_y,deadline_min\n"
        "1,1,0,2,0,10\n"
    ...
    assert payload["swap_model"]["enabled"] is True
    assert payload["swap_model"]["swap_count"] >= 0
    assert "total_lateness_min" in payload
    assert payload["metadata"]["method"] == "C2-Lex-SWAP"
